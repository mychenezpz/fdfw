...
def move_mouse_smoothly(starting_point, firstcontrolpoint, target, steps, kmNet):
    for i in range(steps):
        ...
        if abs(bezierPointX - starting_point[0]) > 127:
            while abs(bezierPointX - starting_point[0]) > 127:
                ...
                kmNet.move(int(step_bezierPointX), 0)
        ...
        if abs(bezierPointY - starting_point[1]) > 127:
            while abs(bezierPointY - starting_point[1]) > 127:
                ...
                kmNet.move(0, int(step_bezierPointY))
...
#Aimbot Tracking 
def AimbotTrackingLegit(head_center_list, FOV_CENTER, ymax, ymin, kmNet):
...
def AimbotTrackingRage(head_center_list, FOV_CENTER, ymax, ymin, kmNet):
    ...
    kmNet.move(int(xdif), int(ydif) + Settings.AntiRecoilValue)
    
    Sleep(0.02) 

#FlickBot
def FlickBot(head_center_list, FOV_CENTER, ymax, ymin, kmNet, send_next, FlickBotCoolDown):
...
def FlickBotLegit(head_center_list, FOV_CENTER, ymax, ymin, kmNet, send_next, FlickBotCoolDown):
    if  GetKeyState(0x02) == True:  
        xdif = (head_center_list[0] - FOV_CENTER[0])*Settings.AimSpeed*0.9
        ydif = (head_center_list[1] - FOV_CENTER[1])*Settings.AimSpeed*0.9                                       
    else:                           
...
def SilentAim(head_center_list, FOV_CENTER, ymax, ymin, kmNet, send_next, FlickBotCoolDown):
    if GetKeyState(0x02) == True:
        xdif = (head_center_list[0] - FOV_CENTER[0])*Settings.AimSpeed*0.9
        ydif = (head_center_list[1] - FOV_CENTER[1])*Settings.AimSpeed*0.9
        ReverseX = -xdif
    ...
    kmNet.move(int(ReverseX), int(ReverseY))
